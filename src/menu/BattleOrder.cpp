...
namespace ProjectSpace
{
	...
	: position{position}, spacing{spacing}, width{0}, height{0}
	{
		...
		for (int i = 1; i < characters.size(); ++i)
		{
			...
			anims.push_back(new TranslateAnimation{portrait, portraitPos, portraits[i - 1]->getPosition(), cycleDuration});
		}
		anims.insert(anims.begin(), new TranslateAnimation{firstPortrait, firstPortrait->getPosition(), portraits[portraits.size() - 1]->getPosition(), cycleDuration});
